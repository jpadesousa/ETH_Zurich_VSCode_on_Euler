...
# Runtime limit default: 1 hour
VSC_RUN_TIME="01:00:00"

# Memory per CPU default: 2048 MB
VSC_MEM_PER_CPU=2048

# Number of GPUs default: 0 GPUs
VSC_NUM_GPU=0

# Waiting interval default: 10 seconds
VSC_WAITING_INTERVAL=10
...
# check if VSC_MEM_PER_CPU is an integer
if ! [[ "$VSC_MEM_PER_CPU" =~ ^[0-9]+$ ]]; then
        echo -e "Error: $VSC_MEM_PER_CPU -> Memory limit must be an integer, please try again\n"
        display_help
else
    echo -e "Memory per CPU set to $VSC_MEM_PER_CPU MB"
fi
...
ssh $VSC_SSH_OPT sbatch <<ENDSBATCH
#!/bin/bash

#SBATCH --ntasks=$VSC_NUM_CPU
#SBATCH --time=$VSC_RUN_TIME
#SBATCH --mem-per-cpu=$VSC_MEM_PER_CPU
...
code-server --extensions-dir=/cluster/work/nme/software/libraries/code-server/4.8.0 --verbose --proxy-domain=http://proxy.ethz.ch:3128 --bind-addr="\${VSC_IP_REMOTE}:8999"
ENDSBATCH
